...
**This alone isn’t enough – you also need conda-forge’s matching
compiler toolchain.** conda-forge’s OpenMPI build bakes in a specific
compiler binary name for its `mpicc` wrapper to call (e.g.
`x86_64-conda-linux-gnu-cc`), which doesn’t exist unless you also
install it. Without it, installing `mpi4py` later fails with something
like:

    The Open MPI wrapper compiler was unable to find the specified compiler
    x86_64-conda-linux-gnu-cc in your PATH.

Fix it by installing conda-forge’s `compilers` meta-package into the
same environment (verified – this is what the error above needs):
...
- **`mpi4py` fails to build with “unable to find the specified compiler
  x86\_64-conda-linux-gnu-cc”** – specific to installing OpenMPI via
  conda-forge; you also need `conda install -c conda-forge compilers -y`
  in the same environment. See the alternative above.
